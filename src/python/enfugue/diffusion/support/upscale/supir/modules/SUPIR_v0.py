...
from enfugue.diffusion.support.upscale.supir.sgm.modules.diffusionmodules.util import (
    avg_pool_nd,
    checkpoint,
    conv_nd,
    linear,
...
from enfugue.diffusion.support.upscale.supir.sgm.modules.diffusionmodules.openaimodel import (
    Downsample,
    Upsample,
    UNetModel,
    Timestep,
...
from enfugue.diffusion.support.upscale.supir.sgm.modules.attention import (
    SpatialTransformer,
...
class ZeroSFT(nn.Module):
    def __init__(self, label_nc, norm_nc, concat_channels=0, norm=True, mask=False):
        ...
        self.mlp_shared = nn.Sequential(
            FastResetConv2d(label_nc, nhidden, kernel_size=ks, padding=pw),
            nn.SiLU()
        )
        self.zero_mul = zero_module(FastResetConv2d(nhidden, norm_nc + concat_channels, kernel_size=ks, padding=pw))
        self.zero_add = zero_module(FastResetConv2d(nhidden, norm_nc + concat_channels, kernel_size=ks, padding=pw))
        # self.zero_mul = FastResetConv2d(nhidden, norm_nc + concat_channels, kernel_size=ks, padding=pw)
        # self.zero_add = FastResetConv2d(nhidden, norm_nc + concat_channels, kernel_size=ks, padding=pw)
    ...
    def __init__(
        ...
        if self.num_classes is not None:
            if isinstance(self.num_classes, int):
                ...
                self.label_emb = FastResetLinear(1, time_embed_dim)
