...
from enfugue.diffusion.support.upscale.supir.sgm.modules.diffusionmodules.sampling_utils import (
    get_ancestral_step,
...
from enfugue.diffusion.support.upscale.supir.sgm.util import (
    append_dims,
    default,
...
DEFAULT_GUIDER = {
    "target": "enfugue.diffusion.support.upscale.supir.sgm.modules.diffusionmodules.guiders.IdentityGuider"
}
...
class EDMSampler(SingleStepDiffusionSampler):
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, callback=None):
...
class LinearMultistepSampler(BaseDiffusionSampler):
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, callback=None, **kwargs):
        x, s_in, sigmas, num_sigmas, cond, uc = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
...
class SubstepSampler(EulerAncestralSampler):
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, control_scale=1.0, *args, callback=None, **kwargs):
        x, s_in, sigmas, num_sigmas, cond, uc = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
...
class RestoreDPMPP2MSampler(DPMPP2MSampler):
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, control_scale=1.0, callback=None, **kwargs):
        x, s_in, sigmas, num_sigmas, cond, uc = self.prepare_sampling_loop(
            x, cond, uc, num_steps
        )
...
class TiledRestoreEDMSampler(RestoreEDMSampler):
    ...
    def __call__(
        ...
        from enfugue.util import get_step_callback
        use_local_prompt = isinstance(cond, list)
        b, _, h, w = x.shape
        latent_tiles_iterator = sliding_window_coordinates(
            h, w, self.tile_size, self.tile_stride
        )
        ...
        if not use_local_prompt:
            ...
            cond_len = len(cond)
            tiles_len = len(latent_tiles_iterator)
            assert cond_len == tiles_len, f"Number of prompts ({cond_len}) should match number of tiles ({tiles_len})"
...
class TiledRestoreDPMPP2MSampler(RestoreDPMPP2MSampler):
    ...
    def __call__(self, denoiser, x, cond, uc=None, num_steps=None, control_scale=1.0, callback=None, **kwargs):
        use_local_prompt = isinstance(cond, list)
        b, _, h, w = x.shape
        latent_tiles_iterator = sliding_window_coordinates(h, w, self.tile_size, self.tile_stride)
        tile_weights = self.tile_weights.repeat(b, 1, 1, 1)
        if not use_local_prompt:
            LQ_latent = cond['control']
        else:
...
def sliding_window_coordinates(h: int, w: int, tile_size: int, tile_stride: int):
