...
class VAEHook:
    def __init__(self, net, tile_size, is_decoder, fast_decoder, fast_encoder, color_fix, to_gpu=False, device='cpu'):
    ...
    def estimate_group_norm(self, z, task_queue, color_fix):
        ...
        for i in range(last_id + 1):
            ...
            if task[0] == 'pre_norm':
                ...
                tile = task[1](tile)
            return torch.any(torch.isnan(tile)).item()
