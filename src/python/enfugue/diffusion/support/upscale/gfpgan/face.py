# type: ignore
# Adapted from https://github.com/xinntao/facexlib/blob/master/facexlib/utils/face_restoration_helper.py
import cv2
import numpy as np
import os
import torch
...
def init_detection_model(model_path, half=False, device="cuda"):
...
def init_parsing_model(model_path, half=False, device="cuda"):
...
class FaceRestoreHelper(object):
    ...
    def __init__(
        ...
        self.face_det = init_detection_model(
            detection_model_path, half=False, device=self.device
        )

        # init face parsing model
        self.use_parse = use_parse
        self.face_parse = init_parsing_model(parse_model_path, device=self.device)
