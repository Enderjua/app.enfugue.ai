...
import sys
import gradio as gr
from enfugue.diffusion.animate.dynamicrafter.scripts.gradio.i2v_test_application import (
    Image2Video,
)
...
def dynamicrafter_demo(result_dir="./tmp/", res=512):
    ...
    with gr.Blocks(analytics_enabled=False, css=css) as dynamicrafter_iface:
        gr.Markdown(
            "<div align='center'> <h1> DynamiCrafter: Animating Open-domain Images with Video Diffusion Priors </span> </h1> \
        ...
        with gr.Tab(label="Generative Frame Interpolation_320x512"):
            with gr.Column():
                ...
                gr.Examples(
                    examples=i2v_examples_interp_512,
                    inputs=[
                        i2v_input_image,
                        i2v_input_text,
                        i2v_steps,
            ...
            with gr.Column():
                with gr.Row():
                    with gr.Column():
                        with gr.Row():
                            i2v_input_image = gr.Image(
                                label="Input Image", elem_id="input_img"
                            )
                        with gr.Row():
                            i2v_input_text = gr.Text(label="Prompts")
                        with gr.Row():
                ...
                gr.Examples(
                    examples=i2v_examples_loop_512,
                    inputs=[
                        i2v_input_image,
                        i2v_input_text,
                        i2v_steps,
