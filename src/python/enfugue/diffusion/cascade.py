...
class UpDownBlock2d(nn.Module):
    def __init__(self, in_channels, c_out, mode, enabled=True):
        super().__init__()
        ...
        mapping = nn.Conv2d(in_channels, c_out, kernel_size=1)
...
class StableCascadeUNet(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        c_clip_text_pooled: Optional[int]=1280,
        c_clip_img: Optional[int] = None,
        c_clip_seq: Optional[int] = 4,
    ...
    def gen_r_embedding(self, r, max_positions=10000):
        ...
        device = r.device
        emb = torch.arange(half_dim, device=device).float().mul(-emb).exp()
        emb = r[:, None] * emb[None, :]
    ...
    def gen_c_embeddings(self, clip_txt_pooled, clip_txt=None, clip_img=None):
        ...
        if clip_txt is not None and clip_img is not None:
            ...
            clip_img = self.clip_img_mapper(clip_img).view(clip_img.size(0), clip_img.size(1) * self.config.c_clip_seq, -1)
    ...
    def forward(self, x, r, clip_text_pooled, clip_text=None, clip_img=None, effnet=None, pixels=None, sca=None, crp=None, **kwargs):
        if pixels is None:
    ...
    @classmethod
    def base_decoder(cls):
        """
        Instantiates a decoder UNet from the base Stable Cascade configuration
        """
        return cls(
            in_channels=4,
            c_out=4,
...
def convert_stable_cascade_unet_state_dict(orig_state_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
    """
    Converts a stable cascade state dictionary into a diffusers one.
    Should function for the prior (stage B) and decoder (stage C)
    """
